<script setup lang="ts">
import {useEnquiryFormHelperService} from '~/services/helpers/enquiry/enquiry-form-helper.service.factory';
import {useEnquirySignInOrSignUpDialog} from '~/services/helpers/auth/sign-in-or-sign-up-dialog.composable';
import type {SignInFormModelValue} from '~/components/auth/SignInForm.vue';
import type {SignUpFormModelValue} from '~/services/helpers/auth/sign-up-strategy';
import {useCurrentLocationService} from '~/services/helpers/current-location.service.factory';

const enquiryFormHelperService = useEnquiryFormHelperService();
const currentLocation = useCurrentLocationService();

const enquiryConfig = ref(null);

const {
  vm,
  formOptions,
  selectedTabChange,
  setTabUrlQueryParam,
  isSignUpFormActive,
  onSignUpStarted,
  loginAfterRegistrationSuccess,
  onSignUpFailed,
  onSignInStarted,
  onSignInSuccess,
  onSignInFailed,
  signUpFormModel,
} = useEnquirySignInOrSignUpDialog(formResultPreparer);


function formResultPreparer(formModelValue: SignInFormModelValue | SignUpFormModelValue) {
  return {
    locationName: currentLocation.getLocationName(),
    eventSummaryNotes: formModelValue?.notesField ?? null,
    packageSettings: {
      peopleCount: formModelValue?.peopleField,
      nightsCount: formModelValue?.nightsField,
      startDate: formModelValue?.dateField,
    },
    productDialogData: this.data.params
  };
}

function needToGetLocationList(): boolean {
  return !!(vm.signUpFormDescription?.locationField ?? vm.signInFormDescription?.locationField);
}


onMounted(async () => enquiryConfig.value = await enquiryFormHelperService.loadDataForEnquiryForm(needToGetLocationList()));

onUpdated(() => {
  setTimeout(() => setTabUrlQueryParam());
});
</script>

<template>
  <the-dialog>
    <template v-slot:header-content>
      <ButtonToggle :itemList="formOptions"
                    v-model="vm.activeForm"
                    @update:modelValue="selectedTabChange">
      </ButtonToggle>
    </template>
    <template #default>
      <div class="sign-in-or-sign-up-form">
        <EnquiryUserSignUpForm ref="signUpForm"
                               v-show="isSignUpFormActive"
                               :form-description="vm.signUpFormDescription"
                               :ip-data="vm.ipData"
                               :form-tracking-params="vm.signUpFormTrackingParams"
                               @action:sign-up-started="onSignUpStarted"
                               @action:sign-up-success="loginAfterRegistrationSuccess"
                               @action:sign-up-failed="onSignUpFailed">
        </EnquiryUserSignUpForm>

        <EnquiryUserSignInForm v-show="!isSignUpFormActive"
                               :disallow-o-auth-google-login="true"
                               :enquiry-sign-up-form-model="signUpFormModel"
                               :form-description="vm.signInFormDescription"
                               :form-tracking-params="vm.signInFormTrackingParams"
                               :pre-populated-email="vm.prePopulatedLoginEmail"
                               @action:sign-in-started="onSignInStarted"
                               @action:sign-in-success="onSignInSuccess"
                               @action:sign-in-failed="onSignInFailed">
        </EnquiryUserSignInForm>
      </div>
    </template>

  </the-dialog>
</template>

<style lang="scss">
</style>
