<template>
    <div v-if="showControls" class="gallery-controls flex flex-col md:flex-row gap-4">
        <!-- Function Control -->
        ...
        <!-- Category Control -->
        <MultiSelect v-if="showCategoryControl" v-model="modelSelectedCategories" display="chip" :options="categoryOptions" optionLabel="name" filter placeholder="Select a Category" :maxSelectedLabels="2" class="category-control w-full md:w-80" />
        ...
        <!-- Card Size Control -->
        <select-button v-if="showCardSizeControl" v-model="modelSelectedSize" :options="filteredCardSizes" optionLabel="label" dataKey="label" aria-labelledby="card-size-selector" class="card-size-control">
