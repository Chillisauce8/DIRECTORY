<template>
  <div ref="selfRef" class="schema-form-array-of-objects-field">
     <div v-if="initDone && sharedFunctions.shouldBeConstructed(props.description.header)"
      ...
      <div class="field_wrap row start-baseline gap-horizontal_15"
           v-for="(row, rowIndex) in vm.model" :key="rowIndex">
  ...
  <!--           @click="showGridView()"></i>-->
      ...
      <div v-if="!ifValidUniqueItems()" class="text-color_red field_wrap">
...
function ifValidUniqueItems(): boolean {
  ...
  return vm.model.length === uniqWith(vm.model, (item: any) => {
