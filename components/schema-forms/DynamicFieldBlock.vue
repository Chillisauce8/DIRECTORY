...
export interface DynamicControlProps extends BaseFieldProps {
    index?: number;
    noWrapper?: boolean;
...
const componentInstance = computed(() => {
    switch (props.description.blockComponent) {
        ...
        case BlockComponents.object:
            return ObjectBlock;
        case BlockComponents.container:
            return ObjectBlock;
        case BlockComponents.arrayOfValues:
            return ArrayOfValuesBlock;
        case BlockComponents.arrayOfObjects:
            return ArrayOfObjectsBlock;
        case BlockComponents.selectOrUploadArray:
            return SelectOrUploadBlock;
    }
});
...
function _getKeyForInnerModel(): string {
    if ([BlockComponents.object, BlockComponents.arrayOfObjects, BlockComponents.arrayOfImages, BlockComponents.arrayOfVideo].includes(props.description.blockComponent)) {
...
function initializeModel() {
    ...
    if (sharedFunctions.shouldSetValueForRealModelValue()) {
        ...
        vm.model = pick(
            parentModel,
            sharedFunctions.getDescription().content.map((item: any) => item.description.name)
        );
