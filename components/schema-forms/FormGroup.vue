      ...
      <template v-for="(contentDescription, contentIndex) in props.description.content">
        <div v-if="im.shouldContentBeConstructed[contentIndex]">
          <template v-if="im.shouldAddHeaderNameToModelPathValues[contentIndex]">
            <DynamicField
              :model="vm.model[props.description.header.name]"
              @modelChange="onModelChangeByPath($event)"
              :context="vm.context"
...
function refreshShouldBeConstructedValues() {
  im.shouldHeaderBeConstructed = sharedFunctions.shouldBeConstructed(props.description.header, vm.context,
    im.shouldHeaderBeConstructed);
  ...
  for (let i = 0; i < props.description.content.length; ++i) {
    const value = sharedFunctions.shouldBeConstructed(props.description.content[i]['description'], vm.context,
      shouldUsePrevValues ? im.shouldContentBeConstructed[i] : null);
...
function initModelIfNeed() {
  if (vm.model) {
    ...
    if (props.description.header.type === 'object' &&
      (!isObject(vm.model[headerName]) || Array.isArray(vm.model[headerName]))) {
