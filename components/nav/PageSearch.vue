<template>
    <client-only>
        <CardsContainer id="search" display="grid" closeButton="true" class="page-search Dark" :class="{ closed: !searchOpen, open: searchOpen }" @closeClicked="$emit('update:searchOpen', false)">
            <template #above-cards>
                <div class="search-container">
                    <div class="search-field-wrapper">
                        <input type="text" name="fname" placeholder="Search" class="search-field" @input="onSearchStringUpdate" />
                    </div>
                </div>
            ...
            <template v-if="searchOpen">
                <template v-for="card in pageCardList">
                    ...
                    <template v-if="card.type === 'post'">
                        <PostCard :link="getPostLink(card.content)" :imageId="card?.content?.feature?.image?.[0]?.image?.id" :title="card?.content?.feature?.title" :search-terms="getCardSearchTerms(card)" :search-hide="getCardSearchHide(card)">
                        </PostCard>
                    </template>
                    ...
                    <template v-if="card.type === 'category'">
                        <CategoryCard class="card" :name="card.content.name" :url="card.content.url" :images="card.content.imageList" :section="getCurrentSection()" :search-terms="getCardSearchTerms(card)" :search-hide="getCardSearchHide(card)">
                        </CategoryCard>
                    </template>
                </template>
            </template>
...
interface PageSearchCardInfo<Type extends 'product' | 'post' | 'category' | 'location', Content extends unknown> {
    type: Type;
    content: Content;
}

type PageSearchProductCard = PageSearchCardInfo<'product', PageSearchProductCardContent>;
type PageSearchProductPost = PageSearchCardInfo<'post', PageSearchPostCardContent>;
type PageSearchProductLocation = PageSearchCardInfo<'location', PageSearchLocationCardContent>;
type PageSearchProductCategory = PageSearchCardInfo<'category', PageSearchCategoryCardContent>;

type PageSearchCard = PageSearchProductCard | PageSearchProductPost | PageSearchProductLocation | PageSearchProductCategory;
    ...
    methods: {
        ...
        getFilteredCards() {
            ...
            return this.pageCardList.filter((c) => {
                ...
                return searchTerms.toLowerCase().indexOf(this.searchString.trim().toLowerCase()) !== -1;
            });
        },
        ...
        getProductSearchTerms(product: Product): string {
            ...
            if (this.currentUser.isStaffOrHiddenStaff()) {
                ...
                const venueNameList = product?.getVenues().map((v) => v.getMainVenue()?.getName() ?? '') ?? [];
