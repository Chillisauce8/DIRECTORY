    ...
    <div class="card">
        <DataView :value="vehicles" paginator :rows="7" layout="grid" :sortOrder="sortOrder" :sortField="sortField">
            ...
            <template #grid="slotProps">
                <layout-grid>
                    <VehicleCardNuxtImage v-for="(item, index) in slotProps.items" :key="index" :name="item.name" :make="item.make" :year="item.year" :images="item.images" :nickname="item.nickname" />
                </layout-grid>
            </template>
        </DataView>
    </div>
